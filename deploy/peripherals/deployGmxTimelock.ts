...
async function runForArbitrum(hre: HardhatRuntimeEnvironment) {
  ...
  const timelock = await deployContract(hre, "GmxTimelock", [
    admin,
    buffer,
    longBuffer,
...
async function runForAvax(hre: HardhatRuntimeEnvironment) {
  const admin = "0x49B373D422BdA4C6BfCdd5eC1E48A9a26fdA2F8b"
...
export default async function (hre: HardhatRuntimeEnvironment) {
  if (network === "avax") {
    await runForAvax(hre)
    return
  }

  await runForArbitrum(hre)
}