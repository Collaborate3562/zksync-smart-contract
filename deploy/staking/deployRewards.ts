...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  const gmx = await deployContract(hre, "GMX", []);
  const esGmx = await deployContract(hre, "EsGMX", []);
  ...
  const feeGmxDistributor = await deployContract(hre, "RewardDistributor", [weth.address, feeGmxTracker.address])
