...
export default async function (hre: HardhatRuntimeEnvironment) {
  const nativeToken = { address: "0x4277f8F2c384827B5273592FF7CeBd9f2C1ac258" }
  ...
  const bnGmx = await deployContract(hre, "MintableBaseToken", ["Bonus GMX", "bnGMX", 0]);
  ...
  const stakedGmxTracker = await deployContract(hre, "RewardTracker", ["Staked GMX", "sGMX"])
  const stakedGmxDistributor = await deployContract(hre, "RewardDistributor", [esGmx.address, stakedGmxTracker.address])
  ...
  const bonusGmxTracker = await deployContract(hre, "RewardTracker", ["Staked + Bonus GMX", "sbGMX"])
  const bonusGmxDistributor = await deployContract(hre, "BonusDistributor", [bnGmx.address, bonusGmxTracker.address])
  ...
  const feeGmxTracker = await deployContract(hre, "RewardTracker", ["Staked + Bonus + Fee GMX", "sbfGMX"])
  const feeGmxDistributor = await deployContract(hre, "RewardDistributor", [nativeToken.address, feeGmxTracker.address])
  ...
  const feeGlpTracker = await deployContract(hre, "RewardTracker", ["Fee GLP", "fGLP"])
  const feeGlpDistributor = await deployContract(hre, "RewardDistributor", [nativeToken.address, feeGlpTracker.address])
  ...
  const stakedGlpTracker = await deployContract(hre, "RewardTracker", ["Fee + Staked GLP", "fsGLP"])
  const stakedGlpDistributor = await deployContract(hre, "RewardDistributor", [esGmx.address, stakedGlpTracker.address])
  ...
  const gmxVester = await deployContract(hre, "Vester", [
    "Vested GMX", // _name
    "vGMX", // _symbol
  ...
  const glpVester = await deployContract(hre, "Vester", [
    "Vested GLP", // _name
    "vGLP", // _symbol
    vestingDuration, // _vestingDuration
    esGmx.address, // _esToken
  ...
  const rewardRouter = await deployContract(hre, "RewardRouterV2", [])
