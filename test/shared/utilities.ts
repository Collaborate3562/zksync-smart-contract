...
export async function gasUsed(provider, tx) {
  return (await provider.getTransactionReceipt(tx.hash)).gasUsed
}

export async function getNetworkFee(provider, tx) {
  const gas = await gasUsed(provider, tx)
  return gas.mul(tx.gasPrice)
}

export async function reportGasUsed(provider, tx, label) {
...
export async function getTxnBalances(provider, user, txn, callback) {
...
export function print(label, value, decimals) {
  if (decimals === 0) {
...
//   for (let i = 0; i < parseInt(((prices.length - 1) / 8).toString()) + 1; i++) {
