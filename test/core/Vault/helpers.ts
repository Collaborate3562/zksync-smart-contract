...
export async function initVaultErrors(vault) {
...
export async function initVaultUtils(vault) {
  const vaultUtils = await deployContract("VaultUtils", [vault.address])
  await vault.setVaultUtils(vaultUtils.address)
  return vaultUtils
}

export async function initVault(vault, router, usdg, priceFeed) {
...
export async function validateVaultBalance(expect, vault, token, offset) {
  if (!offset) { offset = 0 }
...
export function getEthConfig(eth, ethPriceFeed) {
  return [
    eth.address, // _token
    18, // _tokenDecimals
    10000, // _tokenWeight
